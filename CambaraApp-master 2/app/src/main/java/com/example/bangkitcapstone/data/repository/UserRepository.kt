...
import com.example.bangkitcapstone.view.utils.SortUtils
...
class UserRepository private  constructor(
    ...
    fun parseJson(jsonString: String): UploadResponse? {
        val gson = Gson()

        return try {
            gson.fromJson(jsonString, UploadResponse::class.java)
    ...
    fun getAllAccuracyHistory(sortType: AccuracySortType): LiveData<PagedList<AccuracyHistory>> {
        val query = SortUtils.getSortedQuery(sortType)
        val config = PagedList.Config.Builder()
            .setEnablePlaceholders(false)
            .setPageSize(20)
            .build()
        return LivePagedListBuilder(accuracyHistoryDao.getAllAccuracyHistory(query), config).build()
    }
